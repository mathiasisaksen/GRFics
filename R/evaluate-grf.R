...
#' @param grf.object The GRF object containing the realization, generated by \code{\link{generate.grf.object}}.
...
#' @param rescale If true, a min-max scaling is applied to the interpolated values. This ensures that the values are between 0 and 1. Can only be used when \code{uniform.transform = FALSE}.
#' @param uniform.transform If true, a transformation is applied so that the values from realization will follow a uniform distribution. This is done by computing the empirical cumulative distribution function \code{ecdf} of the realization, and then returning \code{ecdf(values)} instead of \code{values}. Can only be used when \code{rescale = FALSE}.
#'
...
evaluate.grf = function(locations, grf.object, realization.number = 1, realization.name = NULL, periodic = TRUE, method = "linear", smooth.func = NULL, rescale = FALSE, uniform.transform = FALSE) {
  if(is.null(grf.object$realizations)) {
    stop("No realizations have been generated.")
  }
  if (all(c(rescale, uniform.transform))) {
    stop("rescale and uniform.transform cannot both be true.")
  }
  ...
  if (!is.null(realization.name)) {
    values = grf.object$realizations[[realization.name]]
  } else {
  ...
  if (uniform.transform) {
    grf.ecdf = ecdf(values)
    interpolated.values = grf.ecdf(interpolated.values)
  }
  if (rescale) {
